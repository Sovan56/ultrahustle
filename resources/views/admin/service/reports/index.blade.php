    ...
    <div class="section-body">
      ...
      <div class="card">
        <div class="card-header">
          <h4 class="m-0">Open/Processed Reports</h4>
        </div>
          ...
          <table id="tbl" class="table table-striped" style="width:100%">
            ...
            <tbody>
              ...
              <tr>
                <td>#{{ $r->id }}</td>
                <td>
                  <a href="{{ route('service.reports.show', $r->id) }}">#{{ $r->service_order_id }}</a>
                </td>
                <td>{{ ucfirst($r->role) }}</td>
                <td>{{ $r->reporter?->email }}</td>
                <td>
                  <span class="badge badge-{{ $r->status === 'open' ? 'warning' : ($r->status === 'approved' ? 'success' : 'secondary') }}">
                    {{ $r->status }}
                  </span>
                </td>
                ...
                <td class="text-right">
                  <a href="{{ route('service.reports.show', $r->id) }}" class="btn btn-sm btn-outline-primary">View</a>
                </td>
              </tr>
              @endforeach
            </tbody>
